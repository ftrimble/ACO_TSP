...
int parse_path_file(char* path_file_path, int num_cities) {
    ...
    // read cities
    int prev_city_number, current_city_number;
    double distance = 0;
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        ...
        fprintf(stderr, "USAGE: %s input_file path_file num_cities\n", argv[0]);
        return EXIT_FAILURE;
    }
    
    int input_parse_return_val, path_parse_return_val;
    sscanf(argv[3], "%d", &num_cities);
    
    distances = alloc2dcontiguous(num_cities, num_cities);
    
    input_parse_return_val = parse_input_file(argv[1], num_cities);
    if (input_parse_return_val != 0) { exit(EXIT_FAILURE); }

    path_parse_return_val = parse_path_file(argv[2], num_cities);
    if (path_parse_return_val != 0) { exit(EXIT_FAILURE); }
